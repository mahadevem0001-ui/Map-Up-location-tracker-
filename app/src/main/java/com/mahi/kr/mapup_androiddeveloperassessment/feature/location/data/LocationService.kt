...
class LocationService : Service() {
            ...
            ACTION_START -> {
                ...
                if (startedFromBoot && Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    ...
                    if (!hasBackgroundPermission) {
                        Log.w(TAG, "onStartCommand: boot restart without background location; ignoring start")
                        stopSelf()
    ...
    private fun startLocationTracking(intervalMs: Long) {
        ...
        locationClient.getLocationUpdates(intervalMs).catch { exception ->
            ...
            getAddressFromLocation(location.latitude, location.longitude) { address ->
                ...
                if (eventTime - lastNotificationUpdateTime >= intervalMs) {
        ...
        val notification = buildNotificationUseCase(
            NotificationConfig(
                ...
                actionTitle = "Stop",
                actionIconRes = android.R.drawable.ic_media_pause,
     ...
     */
    private fun updateNotificationWithLocation(latitude: Double, longitude: Double, address: String?) {
    ...
    private suspend fun createNewSession(): Long {
        ...
        when (val createResult = locationSessionRepository.createSession(newSession)) {
            ...
            is Result.Error -> Log.e(TAG, "createNewSession: failed to create session ${createResult.error}")
        }
    ...
    private fun persistLocation(sessionId: Long, locationData: LocationData) {
        serviceScope.launch {
            when (val result = locationSessionRepository.addLocationToSession(sessionId, locationData)) {
                is Result.Success -> Unit
                is Result.Error -> Log.e(TAG, "persistLocation: failed to save location ${result.error}")
            }
        }
    }
        ...
        serviceScope.launch {
            ...
            active?.let { session ->
                ...
                when (val update = locationSessionRepository.updateSession(ended)) {
                    is Result.Success -> Log.d(TAG, "closeActiveSession: session ${session.sessionId} closed")
                    is Result.Error -> Log.e(TAG, "closeActiveSession: failed to update session ${update.error}")
                }
            }
        }
    }
